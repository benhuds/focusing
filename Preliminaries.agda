...
module Preliminaries where
  ...
  module Nat where
    ...
    iter : {l : Level} {C : Set l} → Nat → C → (Nat → C → C) → C
    iter Z b s = b
    iter (S n) b s = s n (iter n b s)

  open Nat public using (Nat; Z ; S) hiding (module Nat)
  ...
  open Maybe public using (Maybe;Some;None) hiding (module Maybe)
  ...
  module List where
    ...
    peelOff : {A : Set} (eq : DecEq A) → (xs ys : List A) → Maybe (∃[ zs ∈ _ ] ((xs ++ zs) == ys) )
  ...
  module Vector where
    ...
    fromList : {A : Set} → List A → ∃[ n ∈ _ ] Vec A n
    fromList [] = _ , []
    ...
    toList' : {A : Set} → (∃[ n ∈ _ ] Vec A n) → List A
    toList' (._ , []) = []
    ...
    from-to : {A : Set} (l : ∃[ n ∈ _ ] Vec A n) → fromList (toList' l) == l
    from-to (._ , []) = Refl
  ...
  module String where
    ...
    equalb : String -> String -> Bool
    equalb = primStringEquality
    ...
    toVec : String -> ∃[ m ∈ _ ] Vector.Vec Char m
